...
import styles from "./PurchaseHistoryCart.module.css";
import Image from "next/image";
import { Card, CardAction, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "./ui/button";
import { getUserByEmail } from "../../api/user";
import { cancelFlight } from "../../api/flight";

function PurchaseHistoryCart(props) {
    ...
    const handleCancelFlight = async () => {
        if (userId && props.flight.id) {
            try {
                await cancelFlight(userId, props.flight.id);
                alert("Flight cancelled successfully.");
                // Optionally, you can refresh the purchase history or update the state here
            } catch (error) {
                console.error("Error cancelling flight:", error.message);
                alert("Failed to cancel flight. Please try again later.");
            }
        } else {
        ...
        <Card>
                ...
                <CardDescription>
                    <div className={styles.flightTimesContainer}>
                        ...
                        <div className={styles.flightTimes}>
                            ...
                            {new Date(flight.arrivalTime).toLocaleDateString("tr-TR")}
                        </div>
                    </div>
                </CardDescription>
            </CardHeader>
            <CardContent>
                <div className="relative w-[100%] h-[200px]">
                    <Image src={`/assets/airlines/${flight.airline}.jpg`} alt={flight.airline} fill className="object-cover" priority />
                    <div
                        style={{
                            position: "absolute",
                            top: 0,
            ...
            {new Date(flight.departureTime).getDate() < new Date().getDate() ? (
...
export default PurchaseHistoryCart;
